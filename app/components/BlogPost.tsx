    ...
    return (
        <section className="flex flex-col py-20 bg-gray-50 gap-5" id="blog">
            {blogs.map((blog, index) => (
                <div
                key={index}
                className={`container flex flex-col w-full h-fit mx-auto p-4 transition-all duration-300 bg-white rounded-xl ${
                    ...
                    <div className="flex">
                        <Button
                        className="bg-transparent hover:bg-gray-200 text-black text-6xl p-0"
                        onClick={() => handleClick(index)}
                        >
                            {isExpanded && currentBlog == index && isClient ? 
                            <PiCaretUpBold className="w-6 h-6"/> :
